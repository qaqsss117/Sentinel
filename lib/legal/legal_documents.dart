...
const _termsZh = LegalDocument(
  ...
  introduction:
      '欢迎使用哨兵加速器（SentinelVPN）。本协议是您与哨兵加速器运营团队之间关于下载、安装、注册、登录和使用本客户端及相关服务的约定。请在使用前完整阅读并理解本协议，尤其是责任限制、服务中止和争议处理条款。点击“同意并继续”、注册、登录或实际使用服务，即表示您已阅读并同意本协议。',
  sections: [
    LegalSection(
      title: '一、服务内容',
      body:
          '哨兵加速器提供网络连接管理、代理配置、订阅管理、账户管理、套餐购买、设备管理、通知及相关技术服务。具体功能、可用地区、速度、节点和服务期限以客户端及服务端实际提供为准。我们可根据运营、安全、合规或技术需要调整服务，并将在合理范围内提示重要变化。',
    ),
    LegalSection(
      title: '二、账户与安全',
      body:
...
const _privacyZh = LegalDocument(
  ...
  effectiveDate: '生效日期：2026年9月1日',
  introduction:
      '哨兵加速器重视您的个人信息和隐私。本政策说明我们在您使用客户端、账户、订阅、支付和网络连接服务时如何收集、使用、存储、共享和保护信息，以及您如何行使相关权利。协议正文直接内置于客户端，您可随时在“关于”页面查看。',
  sections: [
    LegalSection(
      title: '一、我们收集的信息',
      body:
